...
3. **suggest_template** - Allow Codex to suggest the most appropriate template
...
- Test your setup with Codex
- Replace each stub with your actual implementation
- See helpful hints about what each tool should do

## Design Philosophy

Instead of using rigid rules based on file extensions or patterns, your tools should provide Codex with raw git data and let Codex's intelligence determine:
...
2. Configure the MCP server in Codex :
   ```bash
   # Add the MCP server
   codex mcp add pr-agent -- uv --directory /absolute/path/to/module1/starter run server.py
   
   # Verify it's configured
   codex mcp list
   ```

3. Make some changes in a git repository and ask Codex to analyze your changes and suggest a PR template

## Need Help?

- Check the solution in `../solution/` if you get stuck
- Remember: The goal is to give Codex the data it needs, not to implement complex logic yourself